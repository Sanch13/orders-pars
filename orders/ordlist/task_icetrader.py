...
from ordlist.models import DataIcetrade
from utils.filters import pattern_positive, pattern_negative
from utils.params import url_icetrade, headers
...
for i in range(1, 2):
    ...
    for row in rows:
        ...
        if re.search(pattern_negative, row.a.text):
            continue
        DataIcetrade(name=row.a.text.strip(),
                     link=row.a.get("href"),
                     deadline=row.select_one('td:nth-child(6)').text,
                     price=row.select_one('td:nth-child(5)').text.strip()).save()
        item = {'name': row.a.text.strip(),
                'link': row.a.get("href"),
                'deadline': row.select_one('td:nth-child(6)').text,
